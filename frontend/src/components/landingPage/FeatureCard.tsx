  ...
  return (
    ...
    >
      <Parallax translateX={[-20, 0]} startScroll={startScroll} endScroll={endScroll}
                ...
                bg-slate-100 border-2 border-slate-200 border-opacity-50`}
      >
        <div className={`h-fit flex items-center mb-4`}>
          <h1 className={`text-indigo-600 text-2xl font-medium`}>{title}</h1>
        </div>
      ...
      <Parallax translateX={[80, 0]} startScroll={startScroll} endScroll={endScroll}
        ...
        <img src={`/src/img/${image}`} alt="" className={`object-center `}/>
      </Parallax>
    </div>
